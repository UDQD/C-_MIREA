...
void Tree::show() {
    ...
    printHelper(*Head, " ", false);
}


void Tree::show_tree(Node N) {
    cout << N.x << endl;
    if (N.Less != NULL) {
        this->out_el_for(*N.Less);
    }
...
void Tree::printHelper(Node root, string indent, bool last)
{
    if (&root !=NULL){
        cout << indent;
        if (last) { 
    ...
    //string sColor = Head->color ? "RED" : "BLACK";
        cout << root.x << /*"(" << sColor << ")" << */endl;
        printHelper(*root.Less, indent, false);
        printHelper(*root.More, indent, true);
    }
}
